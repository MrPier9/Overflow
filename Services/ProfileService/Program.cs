...
app.MapGet("/profiles/batch", async (string ids, ProfileDbContext db) =>
{
    ...
    var rows = await db.UserProfiles.Where(x => list.Contains(x.Id))
...
app.MapGet("/profiles", async (string? sortBy, ProfileDbContext db) =>
{
    ...
    if (!string.IsNullOrEmpty(sortBy) && sortBy.Contains("reputation"))
    {
        query = query
...
app.MapPut("/profiles/edit", async (ClaimsPrincipal user, ProfileUpdateDto userProfile, ProfileDbContext db) =>
{
    var userId = user.FindFirstValue(ClaimTypes.NameIdentifier);
    if (userId is null) return Results.Unauthorized();
    var profile = await db.UserProfiles.FindAsync(userId);
