            ...
            <div className="flex flex-col items-end text-sm gap-3 min-w-[6rem]">
                ...
                <div className={clsx('flex justify-end rounded', {
                    ...
                    <span className={clsx('flex items-center gap-2', {
                        ...
                        {question.hasAcceptedAnswer && <CheckIcon className="h-4 w-4 stroke-4" />}
                    ...
                    <div className="flex justify-between pt-2">
                        <div className="flex gap-2">
                            {question.tagSlugs.map(slug => (
                                <Chip key={slug} variant="bordered" as={Link} href={`/questions?tag=${slug}`}>{slug}</Chip>
                            ))}
                        </div>

                        <div className="text-sm flex items-center gap-2">
                            <Avatar className="h-6 w-6" color="secondary" name={question.author?.displayName.charAt(0)} />
                            <Link href={`/profiles/${question.askerId}`}>{question.author?.displayName}</Link>
